...
export function StatsForNerds({ videoRef }: StatsForNerdsProps) {
  const { latency, connectionStatus, participants, playback } = useRoomStore(useShallow(state => ({
  ...
  useEffect(() => {
    ...
    const calculateDrift = () => {
      ...
      const currentDrift = videoRef.current.currentTime - expectedTime;
      setDrift(currentDrift);

      // Update drift every frame for accurate stats
      animationFrameId = requestAnimationFrame(calculateDrift);
    };
