...
## 2024-05-26 - Rate Limiting Bypass via X-Forwarded-For Spoofing
**Vulnerability:** The room creation rate limiter in `server/src/routes/rooms.ts` extracted the client IP using `req.headers['x-forwarded-for']` directly without parsing. An attacker could spoof this header, changing the rate limiter key on every request. This allowed infinite room creation, leading to a Redis OOM (Denial of Service).
**Learning:** Never trust the raw `X-Forwarded-For` header for rate limiting or security checks, as it is easily spoofed by clients. When behind a reverse proxy, rely on framework-level parsed IPs (like Express's `req.ip`) combined with proper `trust proxy` configuration.
**Prevention:** Use `req.ip` for IP-based rate limiting in Express, ensuring `app.set('trust proxy', 1)` is correctly configured to trust the upstream load balancer.
