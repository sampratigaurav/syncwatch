...
## 2024-05-26 - Missing Input Validation in rotate_auth_token Socket Handler
**Vulnerability:** The `rotate_auth_token` socket event handler in `server/src/socket/handlers.ts` destructured `token` directly from the incoming `payload` object (`async ({ token }) =>`) without validating that the payload was not null or undefined. A malicious client could send a `null` payload and crash the entire Node.js server process, causing a Denial of Service.
**Learning:** Always validate that incoming payloads from clients exist and are of the expected type before accessing their properties, even if you are just destructuring them. Socket.IO passes `null` values directly through to the handlers.
**Prevention:** Implement strict payload validation (`if (!payload) return;`) at the very beginning of every Socket.IO event handler that expects data, and avoid destructuring directly in the handler's parameters.
