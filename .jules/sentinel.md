...
## 2024-05-24 - Missing Input Validation on Socket Payloads
**Vulnerability:** Socket.IO payloads lacked strict type and bound checks before being assigned to server state or broadcast to other clients. For example, `playbackRate` was only checked for truthiness, and WebRTC objects were passed blindly.
**Learning:** In highly interactive real-time applications, all incoming socket events must be treated as untrusted input. Relying on client-side TypeScript definitions does not prevent malicious or buggy clients from sending malformed payloads (e.g., passing arrays instead of primitives) which can crash the Node.js server.
**Prevention:** Always validate the type, structure, and bounds of payload properties explicitly on the server before mutating state or emitting to other clients.
