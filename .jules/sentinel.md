...
## 2024-08-01 - Server DoS via Unvalidated Optional Numeric Property
**Vulnerability:** In `server/src/socket/handlers.ts`, the `EVENTS.PLAYBACK_EVENT` handler checked for the existence of `playbackRate` using `!== undefined` before assigning it to the room state. A malicious client could send an object or a massive payload instead of a number, polluting the server state and potentially causing DoS.
**Learning:** Never rely solely on existence checks (`!== undefined`) for optional numeric properties in socket payloads.
**Prevention:** Always validate the type and bounds (e.g. `typeof value === 'number' && Number.isFinite(value)`) of optional payload properties before assignment.
