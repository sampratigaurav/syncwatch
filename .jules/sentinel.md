...
## 2024-05-26 - State Corruption via Unvalidated Playback Rate
**Vulnerability:** In `server/src/socket/handlers.ts`, the `EVENTS.PLAYBACK_EVENT` handler assigns `payload.playbackRate` to server state by checking only `payload.playbackRate !== undefined`. A malicious client could send an object or a string, causing state corruption and potential application crashes or DoS.
**Learning:** When validating optional numeric properties in Socket.IO payloads, do not rely solely on existence checks like `!== undefined`.
**Prevention:** Always explicitly verify the type and bounds (e.g., `typeof value === 'number' && Number.isFinite(value)`) before assigning to server state to prevent state corruption and DoS vulnerabilities.
