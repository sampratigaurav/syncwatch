...
## 2026-06-13 - Server DoS via Null Payload Destructuring in Socket Handler
**Vulnerability:** The `rotate_auth_token` Socket.IO event handler in `server/src/socket/handlers.ts` directly destructured the incoming `payload` object within the function signature (e.g., `async ({ token }) =>`). If a malicious client sent a `null` or `undefined` payload, this destructing attempt would throw a `TypeError`, crashing the Node.js process and causing a Denial of Service. Additionally, it did not perform strict runtime type validation on the `token`.
**Learning:** Destructuring Socket.IO payloads directly in parameter lists is dangerous since unvalidated input can be `null`. A null payload causes an unhandled exception that crashes the entire server.
**Prevention:** Always receive the raw `payload` object in socket handlers, and explicitly perform strict runtime validation (e.g., `if (!payload || typeof payload.token !== 'string') return;`) before accessing or destructuring properties.
