...
## 2024-05-28 - Server DoS and State Corruption via Unvalidated Numeric Property
**Vulnerability:** In `server/src/socket/handlers.ts`, the `EVENTS.PLAYBACK_EVENT` handler checked for the existence of `payload.playbackRate` using `!== undefined` before assigning it to the room state. A malicious client could send an arbitrarily large object or array, which would be assigned to `room.playback.playbackRate` and serialized into Redis, leading to state corruption and a potential Denial of Service (OOM or bloated payloads). It would also be broadcast back to all clients.
**Learning:** Checking for existence (`!== undefined`) is insufficient for optional properties in untrusted payloads. Socket.IO does not enforce types, meaning any JSON-serializable type could bypass this check.
**Prevention:** Always use strict runtime type and bounds validation (e.g., `typeof value === 'number' && Number.isFinite(value)`) for optional numeric properties before assigning them to server state or broadcasting them.
