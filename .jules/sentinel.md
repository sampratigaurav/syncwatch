...
## 2024-08-05 - Socket.IO Payload Validation
**Vulnerability:** Insufficient validation of optional numeric and object properties in Socket.IO event payloads (`playbackRate`, WebRTC `offer`, `answer`, `candidate`).
**Learning:** Socket.IO passes `null` values as objects, and arbitrary data types (like massive strings or objects) can be sent when checking only for `!== undefined`. This can cause server crashes, state corruption, or DoS if these values are processed or broadcasted without strict type and bounds verification.
**Prevention:** Always use explicit type and bounds checking (e.g., `typeof value === 'number' && Number.isFinite(value)`) for numeric inputs, and explicit non-null object checks (`typeof value === 'object' && value !== null`) for object payloads, before handling them or storing them in server state.
