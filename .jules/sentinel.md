...
## 2024-05-26 - State Corruption DoS via Unvalidated Optional Numeric Property
**Vulnerability:** In `server/src/socket/handlers.ts`, the `EVENTS.PLAYBACK_EVENT` handler checked for the existence of `payload.playbackRate` using `!== undefined` but failed to validate its type before assigning it to the room's server state. A malicious client could send an object or large string instead of a number, polluting the state and potentially causing crashes when other clients attempt to consume the corrupted numerical property.
**Learning:** Checking `!== undefined` is insufficient for optional properties. Unvalidated assignments to shared server state create potent Denial of Service and state corruption vectors.
**Prevention:** When validating optional numeric properties in Socket.IO payloads, do not rely solely on existence checks like `!== undefined`. Always explicitly verify the type and bounds (e.g., `typeof value === 'number' && Number.isFinite(value)`) before assigning to server state to prevent state corruption and DoS vulnerabilities.
