...
## 2024-05-26 - Missing Object Validation in WebRTC Payloads
**Vulnerability:** Several Socket.IO event handlers (`WEBRTC_OFFER`, `WEBRTC_ANSWER`, `WEBRTC_ICE_CANDIDATE`) didn't check whether the passed `offer`, `answer`, or `candidate` object in the payload were non-null objects before broadcasting. In another instance (`playbackRate`), the type and bounds of the optional parameter were not verified before saving to the server state.
**Learning:** Checking `!payload` isn't enough, we must check internal destructured parameters are not `null` explicitly, because `typeof null === 'object'`. Optional numeric properties must also explicitly verify their bounds.
**Prevention:** Validate optional numeric properties with `typeof param === 'number' && Number.isFinite(param)` and explicitly ensure objects are not null via `param !== null`.
