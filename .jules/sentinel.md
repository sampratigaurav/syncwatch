...
## 2024-05-24 - DoS via Unvalidated WebRTC Socket.IO Payloads
**Vulnerability:** WebRTC signaling event handlers (`WEBRTC_OFFER`, `WEBRTC_ANSWER`, `WEBRTC_ICE_CANDIDATE`) blindly broadcasted payload properties (`offer`, `answer`, `candidate`) to target clients without verifying if they were objects or null, allowing a malicious client to crash downstream clients or the server logic by sending primitive types or null arrays.
**Learning:** Socket.IO's dynamic typing allows primitive types or `null` to bypass basic TypeScript constraints at runtime. Checking for object types without explicitly discarding `null` is a common JavaScript pitfall that opens DoS vectors.
**Prevention:** Always implement strict runtime type checking for all client-provided objects in WebSocket handlers, specifically including `value !== null` when validating object types, before broadcasting them to other users.
