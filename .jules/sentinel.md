...
## 2024-06-18 - WebRTC Unvalidated Object Types
**Vulnerability:** In `server/src/socket/handlers.ts`, the WebRTC socket handlers (EVENTS.WEBRTC_OFFER, EVENTS.WEBRTC_ANSWER, and EVENTS.WEBRTC_ICE_CANDIDATE) blindly accessed the `payload.offer`, `payload.answer`, and `payload.candidate` properties without validating if they were valid objects or `null`. An attacker could send a `null` object to the target, causing errors on the client when attempting to parse WebRTC structures.
**Learning:** For properties in payloads that should be objects and passed through directly, they must be validated to ensure they are objects and not `null`, since Socket.IO will pass `null` values as 'object' types.
**Prevention:** Always validate nested payload properties with explicit type checks `typeof payload.prop !== 'object' || payload.prop === null`.
