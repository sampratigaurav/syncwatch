...
## 2026-08-27 - Zustand Component Unmounting
**Learning:** Components subscribing to frequently updating Zustand stores (like latency updates every 10s) still execute their hooks and consume resources even if they return `null` based on an `isVisible` prop.
**Action:** Always completely unmount such components using conditional rendering at the parent level (e.g., `{isVisible && <Component />}`) rather than passing visibility props, to prevent unnecessary re-renders and hook execution.
