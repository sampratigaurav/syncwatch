...
## 2025-02-06 - Avoid returning null in Zustand subscribers
**Learning:** Components subscribing to frequently updating Zustand stores (like `useRoomStore`'s `latencyMs`) that return `null` internally when not visible still execute hooks on every store update.
**Action:** Conditionally unmount these components at the parent level (e.g., `{isVisible && <Component />}`) instead of passing an `isVisible` prop to avoid zombie subscriptions and wasteful re-renders.
