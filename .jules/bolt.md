...
## 2026-07-13 - Hidden Component Zustand Subscriptions
**Learning:** Passing visibility props to components that subscribe to frequently updating Zustand stores (like `useRoomStore` tracking `latencyMs`) causes unnecessary re-renders. Early returning `null` inside the component still executes hooks on every store update, degrading performance even when the component is hidden.
**Action:** Completely unmount components that subscribe to high-frequency state using conditional rendering at the parent level (e.g., `{isVisible && <Component />}`) instead of handling visibility internally.
