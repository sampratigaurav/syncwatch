...
## 2024-05-16 - Conditional Mounting for Zustand Subscribers
**Learning:** Passing visibility props (e.g., `isVisible`) to components that subscribe to frequently updating Zustand stores (like `latencyMs` via `useRoomStore`) causes the component's hooks to execute on every store update, even when early returning `null`.
**Action:** Always conditionally render such components at the parent level (e.g., `{isVisible && <Component />}`) to completely unmount them and prevent unnecessary hook executions and virtual DOM evaluations.
