...
## 2024-05-16 - Intl.DateTimeFormat in Render Maps
**Learning:** Calling `toLocaleDateString()` or recreating `Intl.DateTimeFormat` instances inside React render loops (especially inside maps) is a severe performance anti-pattern because formatting object initialization is expensive.
**Action:** Always hoist and cache `Intl.DateTimeFormat` (and similar expensive formatters) outside components and reuse them across renders.
