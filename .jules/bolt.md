...
## 2025-02-13 - Avoid inline `<style>` tags in React components
**Learning:** Using inline `<style>` tags with animations (e.g., `@keyframes`) or global selectors in React components causes unnecessary DOM bloat and layout thrashing on every re-render.
**Action:** Move these non-scoped CSS rules to a global stylesheet like `index.css`.
