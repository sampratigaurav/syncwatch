...
## 2024-05-24 - Avoid returning null internally with frequent store updates
**Learning:** Components subscribing to fast-updating Zustand stores (like latencyMs pinging every 10s) still execute hooks and trigger React's diffing engine even if they return null internally when "hidden".
**Action:** Unmount the component entirely using conditional rendering at the parent level ({isVisible && <Component />}) to prevent these background updates.
