...
## 2024-05-16 - Zustand useRoomStore() Background Hook Execution
**Learning:** Components subscribing to frequently updating Zustand stores (like `useRoomStore` for latency) using early `return null;` based on visibility props will still execute their hooks and cause unnecessary React re-renders in the background on every store update.
**Action:** Always unmount components connected to high-frequency stores using conditional rendering at the parent level (e.g. `{isVisible && <Component />}`) rather than passing visibility down as a prop to early return `null`.
