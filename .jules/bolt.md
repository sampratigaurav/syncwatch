...
## 2024-05-16 - Hoisting Expensive Objects from Render Loops
**Learning:** Recreating expensive instances like `Intl.DateTimeFormat` inside React render loops causes significant performance degradation, particularly in lists/chats where formatters are repeatedly invoked per item, per render.
**Action:** Always hoist and cache these initializations outside components to prevent severe performance degradation on continuous re-renders.
