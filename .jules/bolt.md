...
## 2024-05-16 - Conditional Rendering for Store Subscribers
**Learning:** Components subscribing to frequently updating Zustand stores (like `useRoomStore`) still execute hooks and evaluate state changes even if they return `null` internally when hidden. This causes unnecessary processing in the background.
**Action:** Always unmount the component using conditional rendering at the parent level (e.g., `{isVisible && <Component />}`) instead of passing an `isVisible` prop and returning `null` internally.
