...
## 2024-05-24 - Hoist Expensive Formatters
**Learning:** Recreating expensive instances like `Intl.DateTimeFormat` or calling `new Date().toLocaleDateString()` inside React render loops (especially inside maps) causes severe performance degradation on continuous re-renders.
**Action:** Always hoist and cache these initializations outside components. Wrap values passed to `.format()` in `new Date()` and include falsy fallbacks to prevent `RangeError` crashes.
