...
## 2024-05-16 - Conditional Rendering vs Visibility Props for Zustand
**Learning:** Passing visibility props (like `isVisible`) to components that subscribe to frequently updating Zustand stores (like `latencyMs` in `StatsForNerds`) causes the component to execute hooks on every store update, even when visually hidden (early returning `null`).
**Action:** Always completely unmount the component using conditional rendering at the parent level (e.g., `{showStats && <StatsForNerds />}`) to prevent background hook executions and unnecessary react tree evaluations.
