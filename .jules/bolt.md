...
## 2024-05-16 - Zustand useRoomStore() Subscription while Hidden
**Learning:** Components subscribing to frequently updating Zustand stores (like `useRoomStore`) that receive visibility props (like `isVisible`) still execute their store hooks even when early returning `null`.
**Action:** Conditionally render components at the parent level (e.g., `{isVisible && <Component />}`) instead of passing visibility props down, avoiding unnecessary store updates and component function executions when hidden.
