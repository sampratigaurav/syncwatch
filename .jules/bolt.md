...
## 2026-07-19 - Zustand Visibility Optimization
**Learning:** Passing visibility props to components that subscribe to frequently updating Zustand stores (like `useRoomStore`) is a performance trap. Even if the component early-returns `null`, the hooks still execute on every store update (e.g., latency pings), causing unnecessary render phase execution.
**Action:** Completely unmount the component using conditional rendering at the parent level (e.g., `{isVisible && <Component />}`) instead of passing an `isVisible` prop to avoid subscribing hidden components to the store.
