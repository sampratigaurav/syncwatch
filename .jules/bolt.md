...
## 2024-07-20 - Early Returns with Zustand Subscriptions
**Learning:** Passing visibility props to components that subscribe to frequently updating Zustand stores (like `useRoomStore`) causes hooks to execute on every store update even if the component early returns `null`.
**Action:** Unmount the component entirely using conditional rendering at the parent level (e.g., `{isVisible && <Component />}`) to prevent unnecessary hook execution and re-evaluations.
