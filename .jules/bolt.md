...
## 2024-05-16 - Conditional Rendering vs Early Return
**Learning:** Components that subscribe to frequently updating stores (like latency polling) but early return `null` when hidden still execute their hooks and re-render the function body on every store update.
**Action:** Use conditional rendering at the parent level (`{isVisible && <Component />}`) rather than passing visibility props to heavy components to completely unmount them and prevent unnecessary hook executions.
