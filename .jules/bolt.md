...
## 2026-08-15 - Zustand Component Unmounting vs Internal Null Return
**Learning:** When optimizing React components that subscribe to frequently updating Zustand stores (like `useRoomStore`), passing visibility props and returning `null` internally still executes hooks on every store update, causing unnecessary performance overhead.
**Action:** Always completely unmount the component using conditional rendering at the parent level (e.g., `{isVisible && <Component />}`) to prevent background hook execution.
