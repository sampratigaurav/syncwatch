...
## 2024-05-18 - Unmount frequently updating Zustand components
**Learning:** Components subscribing to frequently updating Zustand stores (like latencyMs which updates every ping) execute their hooks on every update even if they return `null` internally based on a visibility prop. This causes unnecessary react tree traversal.
**Action:** When optimizing React components that subscribe to frequently updating Zustand stores, avoid passing visibility props and returning `null` internally. Instead, completely unmount the component using conditional rendering at the parent level (e.g., `{isVisible && <Component />}`).
