...
## 2024-05-16 - Zustand Subscription in Hidden Components
**Learning:** Components subscribing to frequently updating Zustand stores (e.g., `useRoomStore`) via hooks continue to execute and re-render on every store update even if they return `null` internally based on a visibility prop.
**Action:** Completely unmount the component using conditional rendering at the parent level (e.g., `{isVisible && <Component />}`) instead of passing visibility props to prevent unnecessary hook executions and re-renders in the background.
