...
## 2025-03-05 - Avoid Passing Visibility Props to Store-Connected Components
**Learning:** Components that subscribe to frequently updating Zustand stores (like `useRoomStore` which updates latency often) will continue to execute hooks and re-render on every store update even if they return `null` internally due to an `isVisible` prop.
**Action:** Completely unmount the component using conditional rendering at the parent level (e.g., `{isVisible && <Component />}`) instead of passing visibility props down.
