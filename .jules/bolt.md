...
## 2024-05-16 - Conditional Rendering vs Visibility Props
**Learning:** Components subscribing to frequently updating Zustand stores via hooks will still re-render on store updates even if they return `null` early due to a visibility prop.
**Action:** Avoid passing visibility props to components connected to fast-updating stores. Instead, conditionally render the component at the parent level (e.g. `{isVisible && <Component />}`) to completely unmount it and eliminate background re-renders.
