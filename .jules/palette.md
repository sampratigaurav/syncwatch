...
## 2024-08-07 - Icon Button Accessibility
**Learning:** Icon-only interactive elements in this app's components (like those in ProfileModal) require explicit `aria-label`, `focus-visible` outline styles, and `aria-hidden="true"` on their inner SVG elements to ensure robust screen reader compatibility and keyboard accessibility.
**Action:** Always verify icon-only buttons include these three essential attributes when reviewing or adding new interactive elements.
