...
## 2025-09-11 - Add ARIA Labels to Dashboard Inputs
**Learning:** Found instances where input elements and icon-only buttons in the Dashboard lacked descriptive ARIA labels, making them inaccessible to screen readers. Specifically, the nickname input and the PIN reveal button.
**Action:** Always verify that input fields have programmatic labels (either `<label>` or `aria-label`) and that icon-only interactive elements use `aria-label` while hiding the decorative icon with `aria-hidden="true"`.
