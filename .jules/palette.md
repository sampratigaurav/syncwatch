...
## 2024-05-24 - Dynamic Tooltips for Disabled States
**Learning:** Users often get confused when buttons are disabled without context. Adding a dynamic `title` attribute that changes based on the disabled state provides immediate, native explanatory tooltips.
**Action:** Apply dynamic `title` attributes (e.g., `title={isDisabled ? "Why it is disabled" : "Normal action"}`) and `disabled:cursor-not-allowed` to disabled buttons across the application.
