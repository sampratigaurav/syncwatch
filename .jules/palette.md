...
## 2024-07-14 - CSS-based Tooltips Accessibility
**Learning:** CSS-based tooltips using only `group-hover` are inaccessible to keyboard users, and using `tabIndex={0}` on `<div>` elements for triggers fails SonarCloud accessibility checks.
**Action:** When implementing CSS-based tooltips, pair `group-hover` with `group-focus-within` and use natively focusable semantic elements like `<button type="button">` for the parent trigger.
