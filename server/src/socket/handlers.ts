...
export const setupSocketHandlers = (io: Server) => {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('rotate_auth_token', async (payload: any) => {
    ...
    socket.on(EVENTS.PLAYBACK_EVENT, async (payload: PlaybackEvent) => {
      ...
      if (payload.playbackRate !== undefined) {
        if (typeof payload.playbackRate !== 'number' || !Number.isFinite(payload.playbackRate) || payload.playbackRate < 0.1 || payload.playbackRate > 10) {
    ...
    socket.on(EVENTS.WEBRTC_ICE_CANDIDATE, async (payload: { candidate: any, targetId: string }) => {
      if (!payload || typeof payload.targetId !== 'string') return;
      if (typeof payload.candidate !== 'object') return; // Allow null candidate (End-of-Candidates)
      if (!await getSharedRoom(socket.id, payload.targetId)) return;
